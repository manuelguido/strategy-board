...
export function useExport() {
    ...
    function toMarkdown(): string {
        ...
        lines.push('');
        lines.push(
            `_${state.nodes.length} nodes · ${state.edges.length} edges_`,
        );
        lines.push('');
        ...
        for (const def of NODE_TYPES) {
            ...
            for (const n of group) {
                ...
                if (n.metadata.description) {
                    lines.push(`- ${n.metadata.description}`);
                }

                const md = n.metadata;

                if (md.endpoints?.length) {
                    lines.push(`- **Endpoints:** ${md.endpoints.join(', ')}`);
                }

                if (md.auth) {
                    lines.push(`- **Auth:** ${md.auth}`);
                }

                if (md.runtime) {
                    lines.push(`- **Runtime:** ${md.runtime}`);
                }

                if (md.repo) {
                    lines.push(`- **Repo:** ${md.repo}`);
                }

                if (md.engine) {
                    lines.push(`- **Engine:** ${md.engine}`);
                }

                if (md.tables?.length) {
                    lines.push(`- **Tables:** ${md.tables.join(', ')}`);
                }

                if (md.broker) {
                    lines.push(`- **Broker:** ${md.broker}`);
                }

                if (md.topics?.length) {
                    lines.push(`- **Topics:** ${md.topics.join(', ')}`);
                }

                if (md.schedule) {
                    lines.push(`- **Schedule:** ${md.schedule}`);
                }

                if (md.vendor) {
                    lines.push(`- **Vendor:** ${md.vendor}`);
                }
        ...
        for (const e of state.edges) {
            ...
            if (e.metadata.latencyMs != null) {
                tail.push(`~${e.metadata.latencyMs}ms`);
            }

            if (e.metadata.retry && e.metadata.retry !== 'none') {
                tail.push(
                    `retry:${e.metadata.retry}${e.metadata.retryMax ? `(${e.metadata.retryMax})` : ''}`,
                );
            }
    ...
    async function importFromFile(
        ...
        try {
            ...
            if (
                ...
                return {
                    ok: false,
                    error: 'File is not a valid StrategyBoard JSON.',
                };
            }
